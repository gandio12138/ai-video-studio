...
## 输入
先读 WORKFLOW_ENTRYPOINTS.md。输入为已确认内容依据与逐句台词（A：source_packet；B：剧本）、人物资料、分镜初稿、providers、批准范围。A 不要求额外场景剧本，不调用 script 润色。读 templates/voice_bible.json、tts_job.json 与 contracts/PRODUCTION.md。
## 步骤
...
4. 每句可有多个 take。依模板生成任务，输入包含内容依据/逐句台词表/声音卡/参考哈希；tts_job.script_ref 在 A 指向 source_packet，在 B 指向剧本。未配置接口就 planned/blocked；用户也可在网页生成后手动导入。
5. 真实文件先留原件，再用 probe 实测；统一格式是单独转换步骤。听检漏字/错字/情绪/音色/停顿，有能力才标通过。ASR 回读辅助比对但不自动替代剧本原句。
6. 回写 recorded_audio_duration_s 或独立跨镜音轨计划；声音过长先提出延长镜头或拆镜；任何删词另经用户批准，A 默认保留原句，不私改已确认对白、不默认加速。
