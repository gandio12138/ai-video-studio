...
从小说到成片的 **Codex 驱动项目骨架**：技能说明、项目状态、媒体任务协议、逐句声音与分镜交接、可重建时间线，以及一个可运行的本地基础粗剪后端。
这不是官方漫剧产品，不是已经接通全部服务的自动出片系统，也不保证商业回报。

**先读 START_HERE.md。只需把整个文件夹解压到桌面，在目录里启动 Codex。**
现阶段可继续做小说；配音/视频厂商无需现在确定。

## 能力清单
| 阶段 | Skill | 主要产物 |
|---|---|---|
| 统筹 | manju-producer | 当前阶段、关卡、预算、阻塞、恢复/变更影响 |
| 故事 | manju-story | 小说构思、大纲、人物目标、因果链 |
| 剧本 | manju-script | 场景剧本、对白压缩、稳定台词编号 |
...
## 三种使用方式
**现在写小说：** 用 story/script/audit，不必接任何音视频接口。
...
## 验证
本次版本通过56项单元测试（16项原镜头校验、40项v2），并在本次 Linux 环境完成4秒纯色/测试音的实际渲染测试。测试覆盖硬切、素材裁切、多轨声音定位/淡变、外挂/软字幕、完整解码与拒绝覆盖；不涉及真人或模型配音，不验证你的 Mac、付费接口或故事质量。详见 VALIDATION.md。
